...
$.fn.addtabs = function (options) {
    ...
    var navobj = $(window.top.document).find(options.nav);
    var tabobj = $(window.top.document).find(options.tab);
    ...
    $(options.monitor).on('click', '[addtabs]', function (e) {
        if ($(this).attr('url').indexOf("javascript") !== 0) {
            ...
            if (history.pushState && !$(this).data("pushstate")) {
                window.top.history.pushState(state, title, url);
            }
    ...
    _add = function (opts) {
        ...
        //如果TAB不存在，创建一个新的TAB
        if ($(window.top.document).find("#" + id).size() == 0) {
            //创建新TAB的title
        ...
        //激活TAB
        $(window.top.document).find("#tab_" + id).addClass('active');
        $(window.top.document).find("#" + id).addClass("active");
        // 页签切换滚动条上移
        var tab = $(window.top.document).find("#" + id)
        if (tab.find('iframe')[0]) {
          var fwindow = tab.find('iframe')[0].contentWindow
          fwindow.scrollTo(0, fwindow.scrollY - 1)
        }
        _drop();
    };
    ...
    _drop = function () {
        ...
        //检测是否已增加
        if (!$(window.top.document).find('.tabdrop').html()) {
